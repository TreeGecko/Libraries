...
namespace TreeGecko.Library.Geospatial.Geoframeworks.Objects
{
    ...
    public struct GeographicRectangle : IFormattable, IEquatable<GeographicRectangle>, IXmlSerializable
    {
        ...
        private Latitude m_Bottom;
        private Longitude m_Left;
        private Longitude m_Right;
        ...
        public GeographicRectangle(Position location, GeographicSize size)
        {
            ...
            m_Bottom = location.TranslateTo(Azimuth.South, size.Height).Latitude;
        ...
        public GeographicRectangle(Position location, Distance width, Distance height)
        {
            ...
            m_Top = location.Latitude;
            m_Bottom = location.TranslateTo(Azimuth.South, height).Latitude;
        ...
        public GeographicRectangle(string value, CultureInfo culture)
        {
            ...
            {
                m_Top = Latitude.Empty;
                m_Bottom = Latitude.Empty;
                m_Left = Longitude.Empty;
                m_Right = Longitude.Empty;
            ...
            switch (Values.Length)
            {
                case 4:
                    ...
                    try
                    {
                        ...
                        m_Top = TempTop.DecimalDegrees > TempBottom.DecimalDegrees ? TempTop : TempBottom;
                        m_Left = TempLeft.DecimalDegrees < TempRight.DecimalDegrees ? TempLeft : TempRight;
                        m_Bottom = TempBottom.DecimalDegrees < TempTop.DecimalDegrees ? TempBottom : TempTop;
                        m_Right = TempRight.DecimalDegrees > TempLeft.DecimalDegrees ? TempRight : TempLeft;
        ...
        {
            m_Left = new Longitude(left);
            m_Right = new Longitude(right);
            m_Top = new Latitude(top);
            m_Bottom = new Latitude(bottom);
        ...
        public GeographicRectangle(XmlReader reader)
        {
            ...
            m_Top = Latitude.Invalid;
            m_Bottom = Latitude.Invalid;
            m_Left = Longitude.Invalid;
            m_Right = Longitude.Invalid;
            m_Center = Position.Invalid;
        ...
        public GeographicRectangle CenterOn(Position position)
        {
            ...
            return new GeographicRectangle(
                m_Left.Add(Offset.Longitude),
                m_Top.Add(Offset.Latitude),
                m_Right.Add(Offset.Longitude),
                m_Bottom.Add(Offset.Latitude));
        }
        ...
        public GeographicRectangle UnionWith(Position position)
        {
            ...
            return new GeographicRectangle(
                m_Top.DecimalDegrees > position.Latitude.DecimalDegrees ? m_Top : position.Latitude,
                m_Left.DecimalDegrees < position.Longitude.DecimalDegrees ? m_Left : position.Longitude,
                m_Bottom.DecimalDegrees < position.Latitude.DecimalDegrees ? m_Bottom : position.Latitude,
                m_Right.DecimalDegrees > position.Longitude.DecimalDegrees ? m_Right : position.Longitude);
        }
        ...
        public GeographicRectangle Inflate(Longitude width, Latitude height)
        {
            ...
            // Build a new GeographicRectangle expanded outward from the center
            return new GeographicRectangle(
                m_Left.Subtract(HalfWidthOffset),
                m_Top.Add(HalfHeightOffset),
                m_Right.Add(HalfWidthOffset),
                m_Bottom.Subtract(HalfHeightOffset));
        }
        ...
        public GeographicRectangle IntersectionOf(GeographicRectangle rectangle)
        {
            ...
            if (rectangle.Left >= m_Left && rectangle.Left <= m_Right)
            {
                ...
                if (rectangle.Right >= m_Left && rectangle.Right <= m_Right)
                {
                ...
                // Does the top side of the box fall within the current instance?
                if (rectangle.Bottom <= m_Top && rectangle.Bottom >= m_Bottom)
                {
                    // Yes.  Use the top of the parameter rectangle
        ...
        public bool IsIntersectingWith(GeographicRectangle rectangle)
        {
            ...
            if (rectangle.Left > m_Left && rectangle.Right < m_Right
                && rectangle.Top < m_Top && rectangle.Bottom > m_Bottom)
                return false;
        ...
        {
            return new GeographicRectangle(
                rectangle.Top.DecimalDegrees > m_Top.DecimalDegrees ? rectangle.Top : m_Top,
                rectangle.Left.DecimalDegrees < m_Left.DecimalDegrees ? rectangle.Left : m_Left,
                rectangle.Bottom.DecimalDegrees < m_Bottom.DecimalDegrees ? rectangle.Bottom : m_Bottom,
                rectangle.Right.DecimalDegrees > m_Right.DecimalDegrees ? rectangle.Right : m_Right);
        }
        ...
        public void ReadXml(XmlReader reader)
        {
            ...
            switch (reader.LocalName.ToLower(CultureInfo.InvariantCulture))
            {
                case "envelope":
                    ...
                    // Calculate bounds
                    m_Left = southWest.Longitude < northEast.Longitude ? southWest.Longitude : northEast.Longitude;
                    m_Right = northEast.Longitude > southWest.Longitude ? northEast.Longitude : southWest.Longitude;
                    m_Top = northEast.Latitude > southWest.Latitude ? northEast.Latitude : southWest.Latitude;
                    m_Bottom = southWest.Latitude < northEast.Latitude ? southWest.Latitude : northEast.Latitude;
                ...
                case "box":
                    ...
                    {
                        m_Bottom = Corner1.Latitude;
                        m_Top = Corner2.Latitude;
                    }
                    else
                    {
                        m_Bottom = Corner2.Latitude;
                        m_Top = Corner1.Latitude;
                    }

                    if (Corner1.Longitude < Corner2.Longitude)
                    {
                        m_Left = Corner1.Longitude;
                        m_Right = Corner2.Longitude;
                    }
                    else
                    {
                        m_Left = Corner2.Longitude;
                        m_Right = Corner1.Longitude;
                    }
